...
def main(ad_lib_url) -> None:
    logger.info(f"Pipeline started data collection from {ad_lib_url}")
    raw_data_file_path = collect_raw_data(ad_lib_url)

    logger.info(f"Collection finished, raw data file is here: {raw_data_file_path}")
    transformed_data_file_path = transform_raw_data(raw_data_file_path)

    logger.info(
        f"Transformation finished, clean data file is here: {transformed_data_file_path}"
    )
    report_file_path = generate_report(transformed_data_file_path)

    logger.info(
        f"Report with top 10 performing ads could be found here: {report_file_path}"
    )
